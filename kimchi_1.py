...
def killProcess(removeListenKey):
    if (removeListenKey):
        os.remove('listenKey_kimchi_1.pickle')
    os.system('kill -9 `pgrep -f kimchi_1`')
            ...
            while not var.is_set():
                if (tradePosition.value == 0):
                    try:
                        ...
                        print('Warning  ' + f'{configKimchi.ticker1:<5}', 'Upbit websocket sleep and Re-start Process - kimchi_1 |', now)
                        killProcess(True)
                else:
                    try:
                        recv_data = await websocket.recv()
            ...
            while not var.is_set():
                if (tradePosition.value == 0):
                    try:
                        ...
                        nowRaw = datetime.datetime.now(timezone('Asia/Seoul'))
                        now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
                        print('Warning  ' + f'{configKimchi.ticker1:<5}', 'Binance websoc. sleep and Re-start Process - kimchi_1 |', now)
                        killProcess(True)
                else:
                    try:
                        recv_data = await websocket.recv()
...
def getCurrencyKrw(rateKrw, newRateKrw, tradePosition): 
    while True:
        try:
            ...
            nowRaw = datetime.datetime.now(timezone('Asia/Seoul'))
            now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
            print('Warning Google KRWUSD rate Crawling fail and Pass Process - kimchi_1 |', now)
            pass
...
def getFStream():
    ...
    try:
        ...
        if (not os.path.isfile('orderId_kimchi_1.pickle')):
            print('Warning  ' + f'{configKimchi.ticker1:<5}', 'Binance get ListenKey and Re-start Process - kimchi_1 |', now)
        else:
            print('Warning #' + f'{configKimchi.ticker1:<5}', 'Binance get ListenKey and Re-start Process - kimchi_1 |', now)
        killProcess(True)
    ...
    while True:
        try:
            listenKEY = open('listenKey_kimchi_1.pickle', 'wb')
        except:
            time.sleep(config.errorRetryTimer)
        else:
            break
        ...
        async with websockets.connect(uri) as websocket:
            ...
            while not var.is_set():
                try:
                    ...
                    now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
                    if (not os.path.isfile('orderId_kimchi_1.pickle')):
                        print('Warning  ' + f'{configKimchi.ticker1:<5}', 'Binance streams sleep and Re-start Process - kimchi_1 |', now)
                    else:
                        print('Warning #' + f'{configKimchi.ticker1:<5}', 'Binance streams sleep and Re-start Process - kimchi_1 |', now)
                    killProcess(True)

                try:
                    if ((temp.get('o').get('s') == (configKimchi.ticker1 + 'USDT')) and (temp.get('o').get('x') == 'TRADE') and (temp.get('o').get('X') == 'FILLED')):
                        while True:
                            try:
                                orderID = open('orderId_kimchi_1.pickle', 'wb')
                            except:
                                time.sleep(config.errorRetryTimer)
                            else:
                                break
                    ...
                    elif ((temp.get('o').get('s') == (configKimchi.ticker1 + 'USDT')) and (temp.get('o').get('o') == 'STOP_MARKET') and (temp.get('o').get('X') == 'EXPIRED')):
                        nowRaw = datetime.datetime.now(timezone('Asia/Seoul'))
                        now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
                        print('Error #' + f'{configKimchi.ticker1:<5}', 'Liqudation Position Binance and Kill Process - kimchi_1 |', now)
                        title   = Array('c', b'Liq. Position: #' + str(configKimchi.ticker1).encode("utf-8") + b' ' + str(lightSailsName).encode("utf-8"))
                        message = Array('c', b'Error Liqudation Position Binance and Kill Process - kimchi_1')
...
def keepAlive(ticker, tradePosition, aliveKimchi, aliveOrderK):
    ...
    while True:
        if (tradePosition.value == 0):
            ...
            if ((not os.path.isfile('orderId_kimchi_1.pickle')) and (float(upbitBalance) < configKimchi.kimchiOrderPrice)):
                nowRaw = datetime.datetime.now(timezone('Asia/Seoul'))
                now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
                print('Error  ' + f'{tickerName:<5}', 'Insufficient Upbit KRW fund and Stop Process ∞ kimchi_1 |', now)
                title   = Array('c', b'Insufficient Upbit KRW: ' + str(tickerName).encode("utf-8") + b' ' + str(lightSailsName).encode("utf-8"))
                message = Array('c', b'Insufficient Upbit KRW fund and Stop Process' + ' ∞ '.encode("utf-8") + b'kimchi_1')
                p6 = Process(target = sendGmailReport, args = (title, message))
                p6.start()
                p6.join()
                killProcess(True)

            elif ((not os.path.isfile('orderId_kimchi_1.pickle')) and (float(binanceBalance) < (configKimchi.kimchiOrderPrice / config.futureLeverage / apiGoogle.getGoogleKRWUSD()))):
                nowRaw = datetime.datetime.now(timezone('Asia/Seoul'))
                now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
                print('Error  ' + f'{tickerName:<5}', 'Insufficient Bin. USDT fund and Stop Process ∞ kimchi_1 |', now)
                title   = Array('c', b'Insufficient Bin. USDT: ' + str(tickerName).encode("utf-8") + b' ' + str(lightSailsName).encode("utf-8"))
                message = Array('c', b'Insufficient Bin. USDT fund and Stop Process' + ' ∞ '.encode("utf-8") + b'kimchi_1')
                p6 = Process(target = sendGmailReport, args = (title, message))
                p6.start()
                p6.join()
                killProcess(True)
        ...
        while True:
            try:
                listenKEY = open('listenKey_kimchi_1.pickle', 'rb')
            except:
                time.sleep(config.errorRetryTimer)
            else:
                break
        ...
        try:
            ...
            now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
            if (not os.path.isfile('orderId_kimchi_1.pickle')):
                print('Warning  ' + f'{configKimchi.ticker1:<5}', 'Binance keeped stream and Re-start Process - kimchi_1 |', now)
            else:
                print('Warning #' + f'{configKimchi.ticker1:<5}', 'Binance keeped stream and Re-start Process - kimchi_1 |', now)
            killProcess(True)
        ...
        try:
            ...
            now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
            if (not os.path.isfile('orderId_kimchi_1.pickle')):
                print('Warning  ' + f'{configKimchi.ticker1:<5}', 'Binance Funding-Rates and Re-start Process - kimchi_1 |', now)
            else:
                print('Warning #' + f'{configKimchi.ticker1:<5}', 'Binance Funding-Rates and Re-start Process - kimchi_1 |', now)
            killProcess(True)
        ...
        now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
        if (tradePosition.value == 0):
            print('', now, '|', f'{tickerName:<5}', '| $:', f'{apiGoogle.getGoogleKRWUSD():<7}', '| K:', f'{aliveK:>5}', '% | F:', f'{fundingRate:>6}', '% |', getStatusDW(tickerName), '| kimchi_1')
        elif (tradePosition.value == 1):
            sign = '#'
            print('', now, '|', f'{tickerName:<5}', '| $:', f'{apiGoogle.getGoogleKRWUSD():<7}', '| →K:', f'{aliveK:>5}', '% · O:', f'{aliveO:>5}', '% |', getStatusDW(tickerName), '| F:', f'{fundingRate:>6}', '% | kimchi_1 |', sign)
        else:
            sign = '##'
            print('', now, '|', f'{tickerName:<5}', '| $:', f'{apiGoogle.getGoogleKRWUSD():<7}', '| →K:', f'{aliveK:>5}', '% · O:', f'{aliveO:>5}', '% |', getStatusDW(tickerName), '| F:', f'{fundingRate:>6}', '% | kimchi_1 |', sign)
...
def getTradeInfo():
    try:
        tradeInfo = open('tradeInfo_kimchi_1.pickle', 'rb')
    except:
...
def checkApiKey():
    ...
    try:
        ...
        nowRaw = datetime.datetime.now(timezone('Asia/Seoul'))
        now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
        print('Error get_account_api_permissions() & sys.exit() - kimchi_1')
        sys.exit(now)
    ...
    try:
        if (apiUpbit.upbitApiKey()[2].get('access_key') != config.upbitAccessKey):
            nowRaw = datetime.datetime.now(timezone('Asia/Seoul'))
            now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
            print('Error request.get(access_key) & sys.exit() - kimchi_1')
            sys.exit(now)
    except:
        nowRaw = datetime.datetime.now(timezone('Asia/Seoul'))
        now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
...
if __name__ == "__main__":
    ...
    try:
        ...
        nowRaw = datetime.datetime.now(timezone('Asia/Seoul'))
        now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
        print('Error getGoogleKRWUSD() & sys.exit() - kimchi_1')
        title   = Array('c', b'Error report')
        message = Array('c', b'Error getGoogleKRWUSD() & sys.exit() - kimchi_1')
        p6 = Process(target = sendGmailReport, args = (title, message))
        p6.start()
        p6.join()
        sys.exit(now)
    ...
    while True:
        if (os.path.isfile('listenKey_kimchi_1.pickle')):
            break
        ...
        if ((upbit.value != 0.0) and (binance.value != 0.0) and (rateKrw.value != 0.0)):
            ...
            if ((ticker == configKimchi.preOrder) and (tradePosition.value != 0)):
                if ((kimchi > configKimchi.PreOrderKimchi) and 
                    ...
                    prevKimchi          = 0
                    orderKimchi         = 0
                    os.remove('tradeInfo_kimchi_1.pickle')
            ...
            if ((kimchi < configKimchi.targetOpenKimchi) and (tradePosition.value == 0) and 
                ...
                if (Ret == SUCCESS):
                    ...
                    while True:
                        try:
                            tradeInfo = open('tradeInfo_kimchi_1.pickle', 'wb')
                        except:
                            time.sleep(config.errorRetryTimer)
                        else:
                            break
                    ...
                    if (emptyBalance != 0):
                        ...
                        now = '{0}-{1:>02}-{2:>02} {3:>02}:{4:>02}:{5:>02}'.format(nowRaw.year, nowRaw.month, nowRaw.day, nowRaw.hour, nowRaw.minute, nowRaw.second)
                        if (emptyBalance == 1):
                            print('Error  ' + f'{ticker:<5}', 'Insufficient Bin. USDT fund and Stop Process - kimchi_1 |', now)
                            title   = Array('c', b'Insufficient Bin. USDT: ' + str(ticker).encode("utf-8") + b' ' + str(lightSailsName).encode("utf-8"))
                            message = Array('c', b'Insufficient Bin. USDT fund and Stop Process - kimchi_1')
                        else:
                            print('Error  ' + f'{ticker:<5}', 'Insufficient Upbit KRW fund and Stop Process √ kimchi_1 |', now)
